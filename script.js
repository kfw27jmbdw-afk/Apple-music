...
/* =================  LOCAL STORAGE ENGINE (V29 - FULL AUTO LOGIC) ================= */
...
// 🟢 ASLI FILE IMPORT LOGIC (Full Block)
async function importLocalSong(input) {
    const file = input.files[0];
    if (!file) return;

    if(typeof showTempMessage === "function") showTempMessage(" Extracting Song & Cover...");

    const songName = file.name.replace(/\.[^/.]+$/, "");
    const songId = "local_" + Date.now();
    let extractedImg = defaultImg;
    ...
    reader.onload = async function(e) {
        ...
        // 1.  Binary Scraper: MP3 bytes se photo nikalna
        try {
            for (let i = 0; i < Math.min(bytes.length, 50000); i++) {
                // "APIC" (Attached Picture) tag search
                if (bytes[i] === 65 && bytes[i+1] === 80 && bytes[i+2] === 73 && bytes[i+3] === 67) {
                    for (let j = i; j < i + 500; j++) {
                        // JPEG (FF D8 FF) ya PNG (89 50 4E) signature match
                        if ((bytes[j] === 0xFF && bytes[j+1] === 0xD8) || (bytes[j] === 0x89 && bytes[j+1] === 0x50)) {
                            let imgData = bytes.slice(j, j + 800000); // Max 800KB
                            let type = (bytes[j] === 0xFF) ? 'image/jpeg' : 'image/png';
                            let blob = new Blob([imgData], { type: type });
                            
                            // Photo ko Base64 mein badlo (Persistent)
                            extractedImg = await new Promise(resolve => {
                                const r = new FileReader();
                ...
                if (extractedImg !== defaultImg) break;
            }
        } catch (err) { console.log("Cover extraction failed", err); }

        // 2. IndexedDB (Tijori) mein binary data save karo 
