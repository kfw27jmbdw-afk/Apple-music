...
function loadAndPlayDriveSong(id, info) {
    ...
    audio.src = `https://www.googleapis.com/drive/v3/files/${id}?alt=media&key=${API_KEY}`;
...
async function loadSong(index) {
    ...
    try {
        ...
        // 🟢 TIJORI CHECK: Local gaana refresh ke baad database se uthao
        if (s.isLocal && s.localId) {
            ...
            const request = store.get(s.localId);

            request.onsuccess = () => {
                if (request.result) {
                    // Purana temporary URL clear karo memory ke liye
                    if (audio.src.startsWith('blob:')) URL.revokeObjectURL(audio.src);
                    
                    const fileBlob = request.result.data;
                    decodedURL = URL.createObjectURL(fileBlob);
                    ...
                    console.error("Local file missing in IndexedDB for ID:", s.localId);
                    showTempMessage("File missing in Database!");
                }
            };
            audio.onended = () => nextSong();
            return; //  Local gaana handle ho gaya, aage mat jao
        }

        // 🔵 NORMAL WEB/BASE64 LOGIC (Cloud songs)
    ...
    audio.onended = () => nextSong();
} // 🟢 Function perfect bracket par khatam.
...
/* =================  LOCAL STORAGE ENGINE (V27 + PERSISTENT) ================= */

let musicDB;
const dbRequest = indexedDB.open("AppleMusicLocalDB", 1);

dbRequest.onupgradeneeded = (e) => {
    musicDB = e.target.result;
    if (!musicDB.objectStoreNames.contains("localSongs")) {
        musicDB.createObjectStore("localSongs", { keyPath: "id" });
    }
};
...
// 🟢 ASLI FILE IMPORT LOGIC (Bina Link ke kaam karega)
async function importLocalSong(input) {
    const file = input.files[0];
    if (!file) return;

    if(typeof showTempMessage === "function") showTempMessage(" Saving to Database...");

    const songName = file.name.replace(/\.[^/.]+$/, "");
    const songId = "local_" + Date.now();

    // 1. IndexedDB mein binary data save karo 
    const transaction = musicDB.transaction(["localSongs"], "readwrite");
    const store = transaction.objectStore("localSongs");
    store.put({ id: songId, data: file, name: songName });

    transaction.oncomplete = () => {
        // 2. Playlist Array mein metadata dalo
        const blobURL = URL.createObjectURL(file);
            ...
            "img": defaultImg,
            "isLocal": true,
            "localId": songId // 🟢 YE SABSE ZARURI LINE HAI
        };
...
; 
        ...
        if(typeof showTempMessage === "function") showTempMessage("Song Added Successfully! ");
    };
}

// 🟢 KILL LINK SYSTEM: Purane functions ko blank kar do taaki error na aaye
function openForm() { console.log("Link system is disabled."); }
function addNewSong() { console.log("Use Files instead."); }
// --- IMPORT MENU CONTROL ---

// Menu ko dikhane ke liye
function showImportMenu() {
    const menu = document.getElementById('import-menu-overlay');
    menu.style.display = 'flex';
    // Thoda delay taaki animation smooth dikhe
    setTimeout(() => {
        menu.style.opacity = '1';
    }, 10);
}

// Menu ko chhupane ke liye
function hideImportMenu() {
    const menu = document.getElementById('import-menu-overlay');
    menu.style.opacity = '0';
    setTimeout(() => {
        menu.style.display = 'none';
    }, 300); // CSS transition time ke barabar
}

// Background click karne par menu band ho jaye
